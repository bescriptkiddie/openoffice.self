...
本文件（`selfware.md`）是 **Selfware 协议**的唯一权威源：
- Kernel、Views、Assets、Skills 等均属于运行时实现（runtime），MAY 被替换或由 Agent 重新生成。
- 任何“更新协议”的操作，都 MUST 以本文件为最终落点（并可回滚）。
- 任何“更新文档/数据”的操作，都 MUST 落在实例的 Canonical Data（本 demo 为 `content/`）范围内，而不是写入本文件。
...
说明：
- `selfware.md` 是**协议文本**（protocol authority）。
- `content/` 是**实例数据范围**（data authority / write scope），协作、保存、合并等写入默认都在这里发生。
...
英文翻译（非规范性）：`selfware.en.md`（如有歧义，以本文件为准）。
