    ...
    for line in lines:
        ...
        if line.startswith("## ") and not line.startswith("## ["):
            if current_section and current_articles:
                sections.append({"name": current_section, "articles": current_articles})
        ...
        # Article link (- [title](url))
            ...
            if match:
                ...
                current_articles.append({"title": article_title, "url": article_url})
...
"""
    ...
    for section in data["sections"][:5]:  # Top 5 sections
        if section["articles"]:
            content_parts.append(
                f"- **{section['name']}**: {len(section['articles'])} 条"
            )
...
def write_change_record(article_path: str, date_str: str) -> None:
    """
    Write memory record for an article after saving.

    Creates YAML frontmatter header on first write and appends change records.
    Follows selfware.ts writeChangeRecord format: id, timestamp, actor, intent, paths, summary, rollback_hint.

    Args:
        article_path: Relative path to article (e.g., "content/articles/2026-03-12-每日AI资讯.md")
        date_str: Date in YYYY-MM-DD format
    """
    memory_path = article_path.replace(".md", ".memory.md")
    memory_filepath = PROJECT_ROOT / memory_path
    filename = memory_filepath.name.replace(".memory.md", ".md")
    ...
    if not memory_filepath.exists():
        header_lines = [
            ...
            "selfware:",
            "  role: memory_changes",
            f'  title: "Change Records for {filename}"',
            '  purpose: "对该文件的每次变更记录元数据（可审计、可协作、可回滚）。"',
            f'  scope: "记录 {article_path} 的内容变更；内容可简短但必须可追溯。"',
        ...
        memory_filepath.write_text("\n".join(header_lines), encoding="utf-8")
    else:
        existing = memory_filepath.read_text(encoding="utf-8")
        existing = re.sub(
            r'(^\s*updated_at:\s*)"[^"]*"',
            f'$1"{now_iso}"',
            existing,
        ...
        memory_filepath.write_text(existing, encoding="utf-8")

    id_timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    change_id = f"chg-{id_timestamp}-import"
...
timestamp: "{now_iso}"
actor: "service"
intent: "import_from_ai_news_radar"
paths:
  - "{article_path}"
summary: "导入每日 AI 资讯到本地文档库，来自 ai-news-radar 每日报告"
rollback_hint: "git checkout -- {article_path} && git checkout -- {memory_path}"
```

"""

    memory_filepath.write_text(
        memory_filepath.read_text(encoding="utf-8") + change_record, encoding="utf-8"
    )
...
def save_self_article(content, date_str):
    ...
    # Write memory record after article is saved
    relative_path = f"content/articles/{filename}"
    write_change_record(relative_path, date_str)
    print(f"✅ Memory record: {relative_path.replace('.md', '.memory.md')}")
...
def main():
    ...
    print(
        f"📊 Found {data['total_count']} articles in {len(data['sections'])} sections"
    )
    ...
    print(
        f"   Article: http://115.191.49.124:8888/content/articles/{date_str}-每日AI资讯.md"
    )
