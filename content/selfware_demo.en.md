...
>
> Selfware 定义了面向智能体时代的统一文件协议：在同一个可分发单元（单个文件或 `.self` 包）内，数据、逻辑和视图 MAY 被捆绑在一起；通过分布式、去中心化的智能体协作和自包含的人机交互，形成人↔智能体和智能体↔智能体之间的协作关系；将文件归还给用户，使它们永不落后，能够无限演进。
...
## 1. Single‑File Authority

This file (`selfware.md`) is the only authoritative source of the **Selfware protocol**:
...
Notes:
- `selfware.md` is the **protocol authority**.
- `content/` is the **instance data authority / write scope**. Collaboration, saving, merging, etc. default to writing here.
...
The authoritative Selfware protocol source is selfware.md at the repository root (English translation: selfware.en.md).
Unless marked as "Non-Normative", this document uses RFC-style terms:
```